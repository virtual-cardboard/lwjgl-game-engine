...
import bundle.visuals.display.GUIDisplayer;
import common.DoNothingRunnable;
import common.coordinates.PixelCoordinates;
import common.physics.HasPositionDimensions;

public abstract class AbstractGUI implements Displayable, Clickable, Hoverable, HasPositionDimensions, Iterable<AbstractGUI> {
	...
	private List<AbstractGUI> children;
	...
	public void disable() {
		this.enabled = false;
		for (AbstractGUI child : children) {
			child.disable();
		}
	}
	...
	protected List<AbstractGUI> getChildrenArray() {
		return children;
	}

	public void addChild(AbstractGUI child) {
		children.add(child);
	...
	@Override
	public Iterator<AbstractGUI> iterator() {
